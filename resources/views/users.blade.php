                        ...
                        @if($users->count() > 0)
                            ...
                            <ul id="users">
                                @foreach($users as $user)
                                    <li><span class="label label-info">{{ $user->first_name.' '.$user->last_name }}</span> <a href="javascript:void(0);" class="chat-toggle" data-id="{{ $user->id }}" data-user="{{ $user->name }}">Open chat</a></li>
                                @endforeach
                            </ul>
                        @else
